...
/*                                                                           */
/* Service permettant la création d'un maillage avec une coupe d'un autre    */
/* maillage.                                                                */
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
/*!
 * \brief Service permettant la création d'un maillage avec une coupe d'un autre maillage.
 *
 * Pour chaque plan, une coupe sera réalisée. Toutes les coupes seront stockées dans le maillage
 * créé par ce service. Pour obtenir ce maillage, vous pouvez appeler la méthode \a meshSection()
 * .
 */
...
_fillNodeUID(Int32& sd_nb_node, UniqueArray<NodeIntersection>& new_nodes)
{
  ...
  //
  {
    ...
    {
      UniqueArray<UnknownNodeOrFace> unknown_node;
    ...
    if (!request_uid[my_proc].empty()) {
      ...
      for (Int32 sr = 0; sr < subDomain()->nbSubDomain(); ++sr) {
        ...
        for (Int32 i = 0; i < size; ++i) {
          ...
          // debug() << "[Nœud][" << my_proc << "] Appliquer2"
          //         << " -- UID0 : " << node_on_edge->m_uid_node0
          //         << " -- UID1 : " << node_on_edge->m_uid_node1
          //         << " -- Nouveau UID : " << node_on_edge->m_uid_new_node;
        }
      }
    }
  }
...
_makeUniqueNodeUID(Int32 sd_nb_node, UniqueArray<NodeIntersection>& new_nodes)
{
  IParallelMng* pm = subDomain()->parallelMng();
  ...
  // info() << "[" << pm->commRank() << "] Résultat de la collecte (rectifié) : " << all_nb_node;
...
_fillFaceUID(Int32& sd_nb_face, UniqueArray<FaceLite>& new_faces)
{
...

  {
    ...
    for (auto& elem : new_faces) {
      ...
      if (elem.m_owner_new_face < 0) {
        ...
        if (owner_min == subDomain()->subDomainId()) {
          ...
          // debug() << "[Face][" << my_proc << " -> " << elem.m_owner_new_face << "] Définir UID"
          //         << " -- UID0 : " << elem.m_node0->m_uid_new_node
          //         << " -- UID1 : " << elem.m_node1->m_uid_new_node
          //         << " -- Nouveau UID : " << elem.m_uid_new_face;
        }
        else {
          ...
          // debug() << "[Face][" << my_proc << " -> " << elem.m_owner_new_face << "] Demande1"
          //         << " -- UID0 : " << elem.m_node0->m_uid_new_node
          //         << " -- UID1 : " << elem.m_node1->m_uid_new_node;
        }
      }
        ...
        // debug() << "[Face][" << my_proc << " -> " << elem.m_owner_new_face << "] Demande2"
        //         << " -- UID0 : " << elem.m_node0->m_uid_new_node
        //         << " -- UID1 : " << elem.m_node1->m_uid_new_node;
      }
    }

    {
      ...
      for (Int32 sr = 0; sr < subDomain()->nbSubDomain(); ++sr) {
        ...
        requests[sr * 2] = pm->send(ArrayView{ 1, &size }, sr, false);
        requests[sr * 2 + 1] = pm->send(request_uid[sr], sr, false);
        // debug() << "[Face][" << my_proc << " -> " << sr << "] Requêtes : " << request_uid[sr];
      }
...

  {
    UniqueArray<UniqueArray<Int64>> answers_uid(subDomain()->nbSubDomain());
    {
      UniqueArray<UnknownNodeOrFace> unknown_face;
      debug() << "[Face][" << my_proc << "] Étape 2";
      ...
      for (Int32 sr = 0; sr < subDomain()->nbSubDomain(); ++sr) {
        ...
        for (Int32 ipair_uid = 0; ipair_uid < requested_uid.size(); ipair_uid += 2) {
          ...
          if (pos) {
            ...
            // debug() << "[Face][" << my_proc << " <- " << sr << "] Trouvé"
            //         << " -- UID0 : " << requested_uid[ipair_uid]
            //         << " -- UID1 : " << requested_uid[ipair_uid + 1]
            //         << " -- Nouveau UID : " << new_faces[pos.value()].m_uid_new_face;
          }
          ...
          // la réponse, on va placer les processus en question.
            ...
            // debug() << "[Face][" << my_proc << " <- " << sr << "] NON Trouvé"
            //         << " -- UID0 : " << requested_uid[ipair_uid]
            //         << " -- UID1 : " << requested_uid[ipair_uid + 1];
          }
        }
      ...
      for (Int32 i = 0; i < unknown_face.size(); ++i) {
        ...
        // debug() << "[Face] Infos additionnelles -- Node0UID : " << unknown_face[i].m_node0_uid << " -- Node1UID : " << unknown_face[i].m_node1_uid << " -- Qui : " << who;
        for (auto proc : who) {
    ...
    while (pos < additionnal_answer.size()) {
      ...
      if (owner_min == subDomain()->subDomainId()) {
        ...
        if (sub_additionnal_answer.size() > 1) {
          ...
          // debug() << "[Face][" << my_proc << "] Envoyer"
          //         << " -- UID : " << face_lite.m_uid_new_face
          //         << " -- pour UID Nœud0 : " << face_lite.m_node0->m_uid_new_node
          //         << " -- UID1 : " << face_lite.m_node1->m_uid_new_node
          //         << " -- à : " << sub_additionnal_answer;
      ...
      // Sinon, on doit aller demander le uid au proprio.
        ...
        // debug() << "[Face][" << my_proc << " -> " << face_lite.m_owner_new_face << "] Recevoir UID pour Nœud"
        //         << " -- UID0 : " << face_lite.m_node0->m_uid_new_node
        //         << " -- UID1 : " << face_lite.m_node1->m_uid_new_node;
      }
    }
  ...
  if (need_more_comm) {
    {
      ...
      // On envoie les UID complémentaires.
      debug() << "[Face][" << my_proc << "] Étape 3.2";

      for (Int32 sr = 0; sr < subDomain()->nbSubDomain(); ++sr) {
        if (sr == subDomain()->subDomainId()) {
          // debug() << "[Face][" << my_proc << " -> " << sr << "] Mes RequêtesC : " << request_uid[sr];
          continue;
        }
        if (request_uid[sr].empty()) continue;
        requests[sr] = pm->send(request_uid[sr], sr, false);
        // debug() << "[Face][" << my_proc << " -> " << sr << "] RequêtesC : " << request_uid[sr];
      }
    ...
    if (!request_uid[my_proc].empty()) {
      for (Int32 sr = 0; sr < subDomain()->nbSubDomain(); ++sr) {
        ...
        // debug() << "[Face][" << my_proc << " -> " << sr << "] Taille reçue : " << size;
...
Int32 MeshCutService::
_makeUniqueFaceUID(Int32 sd_nb_face, UniqueArray<FaceLite>& new_faces)
{
  IParallelMng* pm = subDomain()->parallelMng();
...
_compute()
{
  ...
  for (Int32 i = 0; i < m_plans.size(); ++i) {
    ...
    //          << " -- Propriétaire : " << elem.m_new_node->m_owner_new_node
    //          << " -- Pos : " << elem.m_intersection_pos
    //          << " -- Nœud d'arête0 : " << elem.m_new_node->m_uid_node0
    //          << " -- Nœud d'arête1 : " << elem.m_new_node->m_uid_node1;
    // }
    ...
    //           << " -- Propriétaire : " << elem.m_owner_new_face
    //           << " -- Nœud0 : " << elem.m_node0->m_uid_new_node
    //           << " -- Nœud00 : " << elem.m_node0->m_uid_node0
    //           << " -- Nœud01 : " << elem.m_node0->m_uid_node1
    //           << " -- Nœud1 : " << elem.m_node1->m_uid_new_node
    //           << " -- Nœud10 : " << elem.m_node1->m_uid_node0
    //           << " -- Nœud11 : " << elem.m_node1->m_uid_node1;
    // }
  ...
  info() << "Nouveau maillage : " << m_cloned_mesh->name() << " -- NbNœuds : " << m_cloned_mesh->nbNode() << " -- NbMailles : " << m_cloned_mesh->nbCell();
}
...
_addCells(UniqueArray<Int64>& new_cells)
{
  ...
  while (pos0 < new_cells.size()) {
    ...
    pos0 += 1 + static_cast<Int32>(type); // (1)=UID + (type)=UIDNœuds
  ...
  ENUMERATE_ (Node, inode, m_cloned_mesh->allNodes()) {
    ...
    for (const auto& elem : new_nodes) {
      if (elem.m_new_node->m_uid_new_node == uid) {
        ...
        // debug() << "UID Nœud : " << uid << " -- Coordonnée : " << node_coords[inode];
      }
    }
  }
}
