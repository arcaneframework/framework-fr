...
/*                                                                           */
/* Lecture/écriture pour un maillage polyédrique avec le format de fichier   */
/* vtk.                                                                      */
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
: public TraceAccessor
{
 ...
 public:
  ...
  VtkPolyhedralTools::ReadStatus read(IPrimaryMesh* mesh, const String& filename, bool is_parallel_read)
  {
    ...
    // avertissement : la signification de parallel_read n'est pas évidente :
    // si is parallel_read => le maître lit + diffuse, sinon tous les rangs lisent un maillage pré-distribué
    ...
    if (do_read)
      reader = std::make_unique<VtkReader>(filename, m_print_info_level);
  ...
  class Builder : public IMeshBuilder
  {
   public:
    ...
    void allocateMeshItems(IPrimaryMesh* pm) override
    {
      ...
      m_trace_mng->info() << "---Création du maillage polyédrique : " << pm->name() << "---";
      m_trace_mng->info() << "--Lecture du fichier de maillage " << m_read_info.fileName();
...
_fillItemAllocationInfo(ItemAllocationInfo& item_allocation_info, VtkReader& vtk_reader)
{
  ...
  // Crée les familles d'éléments régulières et les connectivités
...
{
  // Enregistrer les informations de variables et de groupes dans un xml pour les rangs non lecteurs
  ...
  if (auto* cell_data = reader.cellData(); cell_data) {
    // Lire les groupes et variables des cellules
    ...
    for (auto array_index = 0; array_index < cell_data->GetNumberOfArrays(); ++array_index) {
      ...
      if (m_print_info_level.print_debug_info) {
        debug(Trace::High) << "Lecture de la propriété " << cell_array->GetName();
  ...
  if (auto* point_data = reader.pointData(); point_data) {
    // Lire les groupes et variables des nœuds
    ...
    for (auto array_index = 0; array_index < point_data->GetNumberOfArrays(); ++array_index) {
      ...
      if (m_print_info_level.print_debug_info) {
        debug(Trace::High) << "Lecture de la propriété " << point_array->GetName();
  ...
  if (auto* face_data = reader.faceData(); face_data) {
    // Lire les groupes et variables des faces
    ...
    for (auto array_index = 0; array_index < face_data->GetNumberOfArrays(); ++array_index) {
      ...
      if (m_print_info_level.print_debug_info) {
        debug(Trace::High) << "Lecture de la propriété " << face_array->GetName();
  ...
  created_infos_str() << "</infos>";
  // Créer des groupes et variables vides dans le sous-domaine non lecteur
...
_createGroup(vtkDataArray* group_items, const String& group_name, IPrimaryMesh* mesh, IItemFamily* item_family, Int32ConstSpan vtkToArcaneLid) const
{
  ...
  if (group_items->GetNumberOfComponents() != 1)
    fatal() << String::format("Impossible de créer un groupe d'éléments {0}. L'information de groupe dans la propriété de données doit être un scalaire", group_name);
  debug() << "Création du groupe " << group_name;
  ...
  if (!GroupDispatcher::Execute(group_items, group_creator))
    ARCANE_FATAL("Impossible de créer le groupe d'éléments {0}. L'information de groupe dans la propriété de données doit être de type entier", group_name);
  debug() << " IDs locaux pour le groupe d'éléments " << group_name << "  " << arcane_lids; // à supprimer
...
_createVariable(vtkDataArray* item_values, const String& variable_name, IMesh* mesh, IItemFamily* item_family, Int32ConstSpan arcane_to_vtk_lids) const
{
  ...
  if (item_values->GetNumberOfTuples() != item_family->nbItem())
    ARCANE_FATAL("Impossible de créer la variable {0}, {1} valeurs sont données pour {2} éléments dans la famille {3}",
                 variable_name, item_values->GetNumberOfTuples(), item_family->nbItem(), item_family->name());
  debug() << "Création de la variable de maillage " << variable_name;
  ...
  // Restriction aux valeurs entières et réelles
  ...
  // Création de la variable scalaire
  ...
  // Création de la variable de tableau
  else { // ArrayVariable
    is_variable_created = ArrayDispatcher::Execute(item_values, array_variable_creator);
  }
  if (!is_variable_created)
    ARCANE_FATAL("Impossible de créer la variable {0}, son type de données n'est pas pris en charge. Seuls les types réels et entiers sont pris en charge", variable_name);
  return var_info;
}
...
VariableRef* _createVar(IMesh* mesh, const String& var_name, const String& var_data_type_name, eItemKind var_kind)
{
  ...
  if (has_error) {
    ARCANE_FATAL("Nom de type de données invalide {0} pour la création de variable dans VtkPolyhedralMeshIOService", var_data_type_name);
  }
  ...
  case DT_Real:
    ...
    break;
  default:
    ARCANE_FATAL("Gère uniquement DT_Int32, DT_Int64, DT_Real dans VtkPolyhedralMeshIOService");
  }
}
...
_createEmptyVariables(IMesh* mesh, const XmlNodeList& item_variables_node, eItemKind item_kind) const
{
  ARCANE_CHECK_PTR(mesh);
  {
    for (XmlNode xnode : item_variables_node) {
      ...
      debug() << "Création de la variable de maillage : " << name;
                   ...
                   IItemFamily* item_family) const
{
  for (XmlNode xnode : groups_node) {
    String name = xnode.attrValue("name");
    info() << "Construction du groupe : " << name;
...
, m_print_info_level{ print_info_level }
{
  ...
  if (filename.empty()) {
    ...
    m_read_status.failure_message = "le nom de fichier pour le maillage vtk polyédrique est vide.";
    return;
  }
  if (filename.endsWith("vtk"))
    ...
    m_read_status.failure = true;
    m_read_status.failure_message = String::format("Extension vtk non prise en charge pour le fichier {0}. Les extensions vtk prises en charge pour les maillages polyédriques sont {1}",
                                                   filename, VtkReader::supportedVtkExtensions());
  }
  // Vérifie si la grille vtk existe et n'est pas vide
  if (!m_vtk_grid) {
    m_read_status.failure = true;
    m_read_status.failure_message = String::format("Impossible de lire le fichier vtk polyédrique {0}. La grille Vtk n'a pas été créée.", filename);
    return;
  }
  if (m_vtk_grid->GetNumberOfCells() == 0) {
    m_read_status.failure = true;
    m_read_status.failure_message = String::format("Impossible de lire le fichier vtk polyédrique {0}. Aucune maille n'a été trouvée.", filename);
    return;
  }
  if (!m_vtk_grid->GetFaces()) {
    m_read_status.failure = true;
    m_read_status.failure_message = String::format("Le fichier vtk de maillage donné {0} n'est pas un maillage polyédrique, impossible de le lire", filename);
    return;
  }
  ...
  // Lecture des informations de face (pour les variables et les groupes) si présentes
  ...
  if (!ifile) {
    m_read_status.info_message = String::format("Information : aucune maille de face fournie {0}{1} (.vtk ou .vtp) pour définir des variables ou des groupes de face.",
                                                faces_filename_and_extension[0],
                                                faces_filename_and_extension[1]);
  }
  else {
    if (m_vtk_face_grid) { // Vérifie si la grille vtk de face existe et n'est pas vide
      if (m_vtk_face_grid->GetNumberOfCells() == 0) {
        m_read_status.failure = true;
        m_read_status.failure_message = m_read_status.failure_message + String::format(" Erreur lors de la lecture des informations de face pour les groupes dans le fichier de maillage {0} ", faces_filename);
      }
      else {
      ...
      m_read_status.failure = true;
      m_read_status.failure_message = m_read_status.failure_message + String::format("Les données de face n'ont pas pu être construites à partir du fichier {0}{1} (.vtk ou .vtu).", faces_filename_and_extension[0], faces_filename_and_extension[1]);
    }
  }
...
cellUids()
{
  ...
  if (m_cell_uids.empty()) {
    ...
    m_cell_node_uids.reserve(10 * m_vtk_grid->GetNumberOfCells()); // prend une moyenne de 10 nœuds par maille
...
nodeUids()
{
  ...
  if (m_node_uids.empty()) {
    ...
    for (int node_index = 0; node_index < nb_nodes; ++node_index) {
      ...
      // L'UID et l'index pourraient différer à l'avenir (ex. lecture parallèle).
      // Cette structure est créée pour être utilisée dans faceUids. Elle devrait être une carte si node_uid n'est plus l'index du nœud
...
faceUids()
{
  // Doit être appelé en premier (aucun travail effectué s'il a déjà été appelé)
  nodeUids();
  ...
  // Ce tableau contient les informations de face par maille (cf. fichier vtk)
  // nb_faces_première_maille nb_nœuds_première_face_première_maille nœud_1 ... nœud_n nb_nœuds_première_face_deuxième_maille etc

  if (!faces) {
    ARCANE_FATAL("Le maillage {0} n'est pas polyédrique : les faces ne sont pas définies", m_filename);
  }
  Int64 face_uid = 0;
  ...
  m_cell_face_uids.reserve(8 * m_cell_uids.size()); // prend une moyenne de 8 faces par maille
  ...
  for (int face_info_index = 0; face_info_index < face_info_size; cell_index++) { // les données de face sont données par maille
    ...
    for (auto face_index = 0; face_index < current_cell_nb_faces; ++face_index, ++global_face_index) {
      ...
      if (!is_face_found) {
        ...
        // ajoute la maille à la connectivité maille-face
        if (is_front_cell) {
          if (m_face_cell_uids[2 * existing_face_index + 1] != NULL_ITEM_UNIQUE_ID) {
            ARCANE_FATAL("Problème d'orientation de face, UID de face {0}, nœuds {1}, même orientation dans la maille {2} et {3}. Changez le fichier de maillage.",
  ...
  // remplis node_face_uids et node_nb_faces à partir de node_faces (forme de tableau [nb_nœuds][nb_faces_connectées])
  m_node_nb_faces.resize(m_node_uids.size(), 0);
  _flattenConnectivity(node_faces.constSpan(), m_node_nb_faces, m_node_face_uids);

  if (m_print_info_level.print_debug_info) {
    std::cout << "================NŒUDS DE FACE ==============" << std::endl;
...
edgeUids()
{
  ...
  nodeUids(); // Doit être appelé en premier. Aucun travail effectué s'il a déjà été appelé
  ...
  m_cell_edge_uids.reserve(20 * m_cell_uids.size()); // choisir une valeur de 20 arêtes par maille
  ...
  for (int face_info_index = 0; face_info_index < face_info_size; ++cell_index) {
    ...
    for (auto face_index = 0; face_index < current_cell_nb_faces; ++face_index, ++global_face_index) {
      ...
      for (int node_index = 0; node_index < current_face_nb_nodes - 1; ++node_index) {
        ...
        mesh_utils::reorderNodesOfFace(current_edge, sorted_edge); // fonctionne pour les arêtes
        auto [is_edge_found, existing_edge_index] = _findFace(sorted_edge, node_edges,
                                                              m_node_uid_to_index,
                                                              m_edge_nb_nodes,
                                                              edge_uid_to_index, edge_offsets, m_edge_node_uids); // fonctionne pour les arêtes
        if (!is_edge_found) {
          ...
          const Int64UniqueArray& face_node_uids)
{
  ...
  // Si la face existe déjà, elle a déjà été enregistrée dans la connectivité des nœuds des faces
...
{
  // remplis nb_items_connectés_par_item_source
  ...
  // remplis le tableau des items connectés
