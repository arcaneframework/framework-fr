...
startProfiling()
{
  ...
  info() << "Démarrage du profilage : use_backtrace=" << m_use_backtrace
    ...
    for (Integer index = function_depth, nb_index = stack_info.nbIndex(); index < nb_index; ++index) {
      ...
      if (func == m_func_map.end()) {
        if (m_current_func_info >= MAX_FUNC) {
          std::cerr << "arcane_papi_handler: MAX_FUNC atteint !\n";
          break;
        }
...
_getFunc(void* addr, FuncAddrInfo& info)
{
  ...
  info.start_addr = 0;
  if (r != 0) {
    // Il est possible que dladdr ne retourne pas d'erreur mais ne trouve pas
    // le symbole. Dans ce cas, essayer de voir s'il s'agit d'un
    // symbole C#.
...
// ****************************************************************************
// * getInfos pour quelqu'un d'autre
// *  [0] 0xb80dd1a3ul
// *  [1] Taille
// *  [2] total_event
// *  [3] total_fp
...
// *     nb_event
// *     total_percent
// *     m_counters[0]: PAPI_TOT_CYC Cycles totaux
// *     m_counters[1]: PAPI_RES_STL Cycles bloqués sur n'importe quelle ressource
// *     m_counters[2]: PAPI_FP_INS Instructions en virgule flottante
// *     strlen(func_name)
// *
// *
// ****************************************************************************
...
dumpJSON(JSONWriter& writer)
{
  ...
  // Écrit la liste des méthodes référencées et leurs noms manglés et démanglés
  {
  ...
  // Écrit la liste des méthodes triées par ordre décroissant des valeurs
  // des compteurs.
  {
    Integer index = 0;
  ...
  {
    // Crée une liste de piles triées par nombre d'événements décroissant.
    UniqueArray<SortedProfStackInfo> sorted_stacks;
    for (const auto& x : global_infos->m_stack_map) {
      const ProfStackInfo& psi = x.first;
      Int64 nb_stack = x.second;
