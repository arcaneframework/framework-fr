...
_createConstraintsLists(Int64MultiArray2& tied_uids)
{
  ...
  // Il est important de définir les contraintes une seule fois !
  m_cells_with_constraints.clear();
  ...
  // Retourne la réduction car nous avons besoin que tous les sous-domaines soient corrects
...
_initArrayCellsWithConstraints()
{
  ...
  // Calculer tied_uids d'abord car nous ne pouvons pas l'utiliser après la redistribution
  // Note : C'est correct car ConstraintList est global, donc non modifié par la redistribution.
  m_mesh->partitionConstraintMng()->computeConstraintList(tied_uids);
  // Assurez-vous que les contraintes sont locales !
  ...
  if (!_createConstraintsLists(tied_uids)) {
    ...
    // Seulement pour la première itération car les contraintes ne sont pas définies au préalable !
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
//< Ajouter une maille NGB si elle n'est pas déjà présente.
        ...
        HashTableMapT<Int32, Int32>& lids, bool special)
{
  ...
  // TODO faire fonctionner hg_contrib à nouveau.
  //contrib[myoffset] = true;

  // Compter la contribution de la maille une seule fois, même si elle apparaît plusieurs fois
  // if (ptrcommWeights) {
                                    ...
                                    bool no_cell_contrib)
{
  ...
  // Calcule d'abord le degré maximum
  if (m_filter_lid_cells[cell.localId()] == eCellReference) {
    ...
    nbFaces = 0;
    // Active la contrainte, mais pas la référence !
    for (Integer j = 1; j < listCell.size(); ++j) {
  ...
  if (m_filter_lid_cells[cell.localId()] == eCellClassical) {
    ...
    // //Maintenant, ajouter la contribution de la maille au poids de l'arête
    ...
    for (Integer j = 0; j < listCell.size(); ++j) {
      ...
      for (Integer z = 0; z < listCell[j].nbFace(); ++z) {
        ...
        // ne considérer que les faces ayant une maille non marquée comme eCellInAConstraint et avec une maille voisine
...
cellsWeightsWithConstraints(Int32 max_nb_weight, bool ask_lb_cells)
{
  ...
  info() << "Nombre de poids " << nb_weight << " / " << nb_criteria;
...
_cellsProjectWeights(VariableCellArrayReal& cellWgtIn, Int32 nbWgt) const
{
  ...
  if (nbWgt > cellWgtIn.arraySize()) {
    ARCANE_FATAL("Demande trop de poids n={0} array_size={1}", nbWgt, cellWgtIn.arraySize());
  }
...
void MeshPartitionerBase::dumpObject(String filebase)
{
  ...
  // ---
  // Envoyer d'abord l'uid du sommet
  ...
  // Astuce : utiliser Real pour éviter les bugs dans pm->recv ...
  // TODO: Corriger ceci.
  ...
  // Envoyer les coordonnées des sommets
  ...
  // Envoyer les relations au maître
