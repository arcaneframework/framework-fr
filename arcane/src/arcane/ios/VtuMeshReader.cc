                    ...
                    const String& dir_name, bool use_internal_partition)
{
  ...
  /*******************************
   *Récupération des UID des nœuds et des mailles *
   *******************************/
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Récupération maintenant des IDs uniques des nœuds ##";
  vtkPointData* allPointData = unstructuredGrid->GetPointData(); // Données associées aux points
  allPointData->Update();
  vtkDataArray* dataNodeArray = allPointData->GetArray("NodesUniqueIDs");
  vtkLongArrayType* nodesUidArray = nullptr;
  if (!dataNodeArray) {
    m_trace_mng->info() << "[readMeshFromVtuFile] Non trouvé, création d'un nouveau";
  ...
  m_trace_mng->info() << "[readMeshFromVtuFile] Récupéré";

  m_trace_mng->info() << "[readMeshFromVtuFile] ## Récupération maintenant des IDs uniques des mailles ##";
  vtkCellData* allCellData = unstructuredGrid->GetCellData(); // Données associées aux points
  allCellData->Update();
  ...
  if (data_owner_array) {
    if (data_owner_array->GetDataType() == VTK_INT) {
      ...
      m_trace_mng->warning() << "Le tableau NodesOwner est présent mais a un mauvais type (pas Int32)";
    }
  }
  m_trace_mng->info() << "[readMeshFromVtuFile] Récupéré";
  ...
  // Obtenir la dimension du maillage
  ...
  // Créer une table de hachage pour le propriétaire des nœuds.
  ...
  for (Integer i = 0; i < nbOfCells; ++i)
	...
	// Calcule le nombre de mailles/noeuds
   ...
   ******************/
  // Pour toutes les mailles découvertes
  for (Integer i_cell = 0, s_cell = cells_filter.size(); i_cell < s_cell; ++i_cell) {
    ...
    case (VTK_QUAD):
      if (mesh->dimension() == 2) {
        ...
        ARCANE_FATAL("VTK_QUAD n'est pas pris en charge dans les maillages unidimensionnels de dimension {0}",
                     mesh->dimension());
      }
      break;
    case (VTK_TRIANGLE):
      if (mesh->dimension() == 2) {
        ...
        ARCANE_FATAL("VTK_TRIANGLE n'est pas pris en charge dans les maillages unidimensionnels de dimension {0}",
                     mesh->dimension());
      }
      break;
...
#warning IT_Vertex vs IT_DualNode HACK
    ...
    auto nNodes = cell->GetNumberOfPoints(); // Retourne le nombre de points dans la maille

    // Premièrement, c'est le TYPE de la maille. Stocke le type de la maille
    cells_infos[cells_infos_index++] = arcItemType;

    // Ensuite vient son UniqueID. Stocke l'ID unique de la maille
    Integer cell_indirect_id = cells_filter[i_cell];
    cells_infos[cells_infos_index++] = cellsUidArray->GetValue(cell_indirect_id);

    // Et enfin les IDs uniques des nœuds
    vtkIdList* nodeIds = cell->GetPointIds();
    for (int iNode = 0; iNode < nNodes; ++iNode) {
      auto localId = nodeIds->GetId(iNode);
      long uniqueUid = nodesUidArray->GetValue(localId);
      //info() << "traitement du localId=" << localId << ", uniqueUid=" << uniqueUid;
      cells_infos[cells_infos_index++] = uniqueUid;
    }
  }

  /********************************
   * Définition de la dimension et allocation *
   ********************************/
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Maillage 3D ##";
  PRIMARYMESH_CAST(mesh)->setDimension(3);
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Allocation de " << cells_filter.size() << " mailles ##";
  PRIMARYMESH_CAST(mesh)->allocateCells(cells_filter.size(), cells_infos, false);

  // Positionne les propriétaires des nœuds à partir des groupes de nœuds
  ...
  if (nodes_owner_array) {
    m_trace_mng->info() << "Définit les propriétaires des nœuds à partir du fichier vtu";
  ...
  /********************************************
   * Finalisation et préparation pour le maillage fantôme *
   *******************************************/
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Fin avec endAllocate ##";
   ...
   *********************/
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Récupération des autres champs ##";
  ...
  for (int i = 0; i < nbrOfArrays; ++i) {
    if (itWasAnArcanProduction == false)
      continue;
    m_trace_mng->info() << "[readMeshFromVtuFile] Focalisation sur \"" << allFieldData->GetArrayName(i) << "\" (i="
                        << i << ")";
  ...
  /*******************
   * Maintenant insérer les coordonnées *
   *******************/
  m_trace_mng->info() << "[readMeshFromVtuFile] ## Maintenant insérer les coordonnées ##";
  // Remplit la variable contenant les coordonnées des nœuds
  ...
  /****************************************
   * Synchronisation des groupes/variables et des nœuds *
   ****************************************/
  ...
  **************/
  ...
  // TODO: étudier la destruction automatique
  if (!dataNodeArray)
...
{
  //m_trace_mng->info() << "[allowExtension] Vérification de l'extension de fichier...";
  for (vtkFileExtIdx = 0; vtkFileExtReader[vtkFileExtIdx].ext != nullptr; ++vtkFileExtIdx) {
    //m_trace_mng->info() << "Test de '" << vtkFileExtReader[vtkFileExtIdx].ext << "'...";
    if (str == vtkFileExtReader[vtkFileExtIdx].ext) {
      return true;
    }
  }
  //m_trace_mng->info()<<"Échec pour tous!";
  // Définit notre index pour une gestion de service ultérieure
  vtkFileExtIdx = 0;
  return false;
}
                 ...
                 bool use_internal_partition)
{
  ...
  info() << "[readMeshFromFile] Transfert à vtkFileExtReader[" << vtkFileExtIdx << "].reader";
  ...
  : public IMeshBuilder
  {
   ...
   public:
    ...
    void allocateMeshItems(IPrimaryMesh* pm) override
    {
      ...
      m_trace_mng->info() << "Lecteur Vtu (ICaseMeshReader) file_name=" << fname;
      bool ret = vtu_service.readMeshFromVtuFile(pm, fname, m_read_info.directoryName(),
                                                 m_read_info.isParallelRead());
      if (ret)
        ARCANE_FATAL("Impossible de lire le fichier VTK");
    }
