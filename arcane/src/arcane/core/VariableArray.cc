...
copyItemsValues(Int32ConstArrayView source, Int32ConstArrayView destination)
{
  ARCANE_ASSERT(source.size() == destination.size(),
                ("Impossible de copier : les tailles de la source et de la destination sont différentes !"));
...
{
  if (isPartial()) {
    debug(Trace::High) << "Ignorer la compaction pour la variable partielle " << name();
    return;
  }
