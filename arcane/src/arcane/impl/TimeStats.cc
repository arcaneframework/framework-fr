...
{
  info(4) << "Sauvegarde des valeurs TimeStats";
  ...
  if (is_verbose) {
    info() << "Sauvegardé " << action_save_info;
  }
...
{
  info(4) << "Fusion des valeurs TimeStats";
  ...
  if (v == 0)
    return;
  if (v != 1) {
    info() << "Avertissement : impossible de fusionner les valeurs de statistiques de temps car la version du point de contrôle n'est pas compatible";
    return;
  }
