...
class IComplex1Interface
{
 public:
  ...
  virtual ~IComplex1Interface() = default;
...
class IComplex2Interface
{
 public:

  virtual ~IComplex2Interface() {}
};
class IComplex3Interface
{
 public:

  virtual ~IComplex3Interface() {}
};
class IComplex4Interface
{
 public:

  virtual ~IComplex4Interface() {}
};

namespace ArcaneTest
{
class ICaseOptionTestInterface
{
 public:

  virtual ~ICaseOptionTestInterface() {}
};
} // namespace ArcaneTest
...
arcaneLoop2()
{
  ...
  if (m_arcane_loop_done)
    ARCANE_FATAL("Seul ce point d'entrée doit être appelé");
}
...
build()
{
  ...
  if (options()->testId() == 4) {
    ...
    info() << "DéfinirValeurParDéfaut pour CellGroup";
 ...
 public:
  ...
  void checkSubMesh(const String& mesh_name) override
  {
    ...
    if (s->mesh()->name() != mesh_name)
      ARCANE_FATAL("Mauvais maillage attendu={0} valeur={1}", mesh_name, s->mesh()->name());
    const auto& multi_opt = options()->multiPostProcessor;
    if (multi_opt.size() != 2)
      ARCANE_FATAL("Mauvais nombre de multi-post-processeur d'option : v={0} attendu=2", multi_opt.size());
    for (const auto& x : multi_opt) {
      ...
      if (s2->mesh()->name() != mesh_name)
        ARCANE_FATAL("Mauvais maillage attendu={0} valeur={1}", mesh_name, s2->mesh()->name());
    }

    CellGroup group = options()->complex1.cellGroup();
    if (group.name() != "Planchere")
      ARCANE_FATAL("Mauvais groupe attendu='Planchere' valeur='{0}'", group.name());
    if (s->mesh()->name() != group.mesh()->name())
      ARCANE_FATAL("Mauvais maillage pour le groupe attendu='{0}' valeur='{1}'", s->mesh()->name(), group.mesh()->name());
  }
