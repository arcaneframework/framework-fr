...
executeTest()
{
  ...
  _dumpConnections();
  {
    info() << " ** ** VÉRIFIER LA MISE À JOUR COUCHE GHOST";
  ...
  if (mesh_dim == 2 || (mesh_dim == 3 && options()->createEdges())) {
    // Appelle la méthode 2 fois pour vérifier que le recalcul est correct.
...
_testItemPartialAdjacency()
{
  ...
  ENUMERATE_ITEMPAIR(Cell, Cell, icell, center_neighbor_pairgroup)
  {
    ...
    if (item != center_cell)
      fatal() << "Inconsistent center cell";
...
: public Cell
{
 public:
  ...
  explicit MyCellClass(Item aitem)
  : Cell(aitem)
  {}
};
...
_testSharedItems()
{
  info() << "Vérification des éléments partagés";
  ...
  ENUMERATE_ITEM (iitem, cell_family->allItems()) {
    ...
    if (counter > 1 && !item.isShared())
      ARCANE_FATAL("Item non marqué comme élément partagé item={0} compteur={1} is_shared={2}",
                   ItemPrinter(item), counter, item.isShared());
    if (counter == 1 && item.isShared())
      ARCANE_FATAL("Item marqué comme élément partagé item={0} compteur={1} is_shared={2}",
                   ItemPrinter(item), counter, item.isShared());
  }
    ...
    for (Face face : node.faces()) {
      if (!previous_face.null()) {
        if (previous_face.uniqueId() > face.uniqueId())
          ARCANE_FATAL("Les faces connectées ne sont pas triées node={0} previous_face={1} current_face={2}",
...
_testFaces()
{
  ...
  ENUMERATE_ (Cell, icell, allCells()) {
    ...
    // Pour maintenir la compatibilité entre les versions 3.8+ et les versions antérieures
    // nous devons être capables de convertir un 'ItemConnectedListView' en un 'ItemVectorView'
    // et inversement pour les itérateurs
...
_testCoherency()
{
  ...
  if (is_amr && mk.meshAMRKind() == eMeshAMRKind::None)
    ARCANE_FATAL("Incohérence AMR: is_amr==true mais eMeshAMRKind == None");
  if (!is_amr && mk.meshAMRKind() != eMeshAMRKind::None)
    ARCANE_FATAL("Incohérence AMR: is_amr==false mais eMeshAMRKind != None");
}
...
_testFindOneItem()
{
  ...
  // Teste l'entité nulle
  {
    ...
    if (!cell.null())
      ARCANE_FATAL("Maille nulle attendue mais maille trouvée={0}", ItemPrinter(cell));
  }
  ...
  for (Int32 i = 0, n = unique_ids.size(); i < n; ++i) {
    ...
    if (cell1.null())
      ARCANE_FATAL("Maille nulle inattendue");
    if (cell1.localId() != local_ids[i])
      ARCANE_FATAL("Mauvais ID local cell1={0} expected_lid={1}", ItemPrinter(cell1), local_ids[i]);
    Cell cell2 = MeshUtils::findOneItem(cell_family, true_uid);
    if (cell1 != cell2)
      ARCANE_FATAL("Maille différente inattendue cell1={0} cell2={1}", ItemPrinter(cell1), ItemPrinter(cell2));
  }
}
...
_testEvents()
{
  ...
  if (!has_call_begin_prepare_dump)
    ARCANE_FATAL("L'événement BeginPrepareDump n'a pas été appelé");
  if (!has_call_end_prepare_dump)
    ARCANE_FATAL("L'événement EndPrepareDump n'a pas été appelé");
}
...
_testNodeNodeViaEdgeConnectivity()
{
  ...
  ENUMERATE_ (Node, inode, ownNodes()) {
    ...
    if (nb_edge != nb_connectivity_node)
      ARCANE_FATAL("Mauvais nombre de nœuds connectés uid={0} nb_edge={1} nb_connected={2}",
    ...
    for (Int32 i = 0; i < nb_edge; ++i) {
      ...
      if (ref_cx_node_lid != cx_node_lid)
        ARCANE_FATAL("Mauvais nœud connecté uid={0} index={1} ref={2} current={3}",
...
_testBoundaryNodeNodeViaEdgeConnectivity()
{
  info() << "CRÉER Maillage d'arête de nœud de frontière";
...
_testLocalIdsFromConnectivity()
{
  info() << "Test des LocalIdsFromConnectivity";
  ...
  ENUMERATE_ (Cell, icell, mesh()->allCells()) {
    for (Face face : icell->faces()) {
      if (face.localId() != faces_local_id[index])
        ARCANE_FATAL("Mauvaise valeur face={0} expected_local_id={1}", ItemPrinter(face), faces_local_id[index]);
      ++index;
    }
  }
}
