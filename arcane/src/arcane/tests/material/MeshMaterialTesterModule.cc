...
compute()
{
  ...
  // Supprime des mailles pour test
  {
    ...
    info() << "Suppression des mailles n=" << remove_lids.size();
    ...
    if (parallelMng()->isParallel()) {
      ...
      mesh()->modifier()->endUpdate();
      {
        ...
        mmim.beginUpdate();
        info() << "TEST_MATERIAU_MAILLAGE: Mise à jour des couches fantômes";
...
_checkFillPartialValues()
{
  ...
  info() << "Vérification de MaterialVariableCellReal";
  ...
  info() << "Vérification de EnvironmentVariableCellReal";
  ...
  if (m_material_mng->isAllocateScalarEnvironmentVariableAsMaterial()) {
    ...
    info() << "OK pour créer une variable Matériau avec le même nom que la variable Milieu";
  }
  ...
  info() << "Vérification de EnvironmentVariableCellArrayReal";
...
{
  info() << "Vérification fillPartialValuesWithGlobalValues()";
  ...
  info() << "Vérification fillPartialValuesWithSuperValues(LEVEL_ALLENVIRONMENT)";
  _fillVar(mat_var, 7.0);
  mat_var.fillPartialValuesWithSuperValues(LEVEL_ALLENVIRONMENT);
  _checkFillPartialValuesWithGlobal(mat_var, m_material_mng->components());

  info() << "Vérification fillPartialValuesWithSuperValues(LEVEL_ENVIRONMENT)";
  ...
  info() << "Vérification fillPartialValuesWithSuperValues(LEVEL_MATERIAl)";
...
_checkFillPartialValuesWithGlobal(const VarType& var_type, MeshComponentList components)
{
  ...
  ENUMERATE_COMPONENT (ic, components) {
    ...
    ENUMERATE_COMPONENTCELL (iccell, c) {
      ...
      vc.areEqual(my_value, ref_value, "Mauvaise valeur de remplissage avec global");
    }
  }
}
...
_checkFillPartialValuesWithSuper(const VarType& var_type, MeshComponentList components)
{
  ...
  ENUMERATE_COMPONENT (ic, components) {
    ...
    ENUMERATE_COMPONENTCELL (iccell, c) {
      ...
      auto ref_value = var_type[c];
      auto my_value = var_type[iccell];
      vc.areEqual(my_value, ref_value, "Mauvaise valeur de remplissage avec super");
    }
  }
}
                           ...
                           Int64 iteration, bool is_check)
{
  ...
  info() << "SET_OR_CHECK taille1=" << spectral1_dim2 << " taille2=" << spectral2_dim2;
  ...
  FunctorType check_func = [&](Int64 uid, Int64 iteration, ComponentItemLocalId var_index) {
    ...
    if (vc.nbError() != 0) {
      error() << "Erreur pour la maille uid=" << uid << " var_index=" << var_index;
      vc.throwIfError();
    }
  };
...
_checkArrayVariableSynchronize()
{
  info() << "_checkArrayVariableSynchronize(): SYNCHRONISER_MATERIAUX";
...
_computeDensity()
{
  ...
  // Calcul les mailles dans lesquelles il faut ajouter ou supprimer des matériaux
  {
    ...
    ENUMERATE_ENV (ienv, m_material_mng) {
      ...
      ENUMERATE_MAT (imat, env) {
        ...
        info() << "INFO_DENSITE_REMPLISSAGE ITER=" << m_global_iteration()
               << " mat=" << mat->name()
               << " nb_a_ajouter=" << mat_to_add_array.size()
               << " nb_a_supprimer=" << mat_to_remove_array.size();
  ...
  m_material_mng->synchronizeMaterialsInCells();
  info() << "Synchronisation de la densité";
             ...
             bool is_compute_mat)
{
  ...
  info() << "REMPLISSAGE MAT=" << mat->name();

  // Copie dans tmp_cell_mat_density la valeur partielle de la densité pour \a mat
  ...
  // Phase 1, calcule les mailles où le matériau sera créé ou supprimé.
  ...
  if (is_compute_mat) {
    ENUMERATE_CELL (icell, allCells()) {
      ...
      if (new_density >= 0.5 && !has_material && current_density == 0.0) {
        ...
        info() << "NOUVELLE MAILLES POUR MAT " << mat_id << " uid=" << ItemPrinter(cell) << " nouveau=" << new_density;
      }
      else if (new_density < 0.4 && has_material && current_density > 0.4) {
        mat_to_remove_array.add(cell.localId());
        info() << "SUPPRESSION DE MAILLES POUR MAT " << mat_id << " uid=" << ItemPrinter(cell) << " nouveau=" << new_density
               << " ancien=" << current_density;
      }
      tmp_cell_mat_density[icell] = new_density;
    }
  }
  else {
    // Phase 2: met à jour les valeurs maintenant que le matériau a été
    ...
    ENUMERATE_MATCELL (imatcell, mat) {
      ...
      //info() << "ASSIGN maille=" << global_cell.uniqueId() << " densité=" << density;
...
_checkCreation()
{
  ...
  info() << "VÉRIFICATION CRÉATION N=" << n;
  ...
  ValueChecker vc(A_FUNCINFO);
  {
    ...
    ENUMERATE_MATCELL (imatcell, m_mat1) {
      MatCell mmc = *imatcell;
      vc.areEqual(mat_pressure[mmc], mat_pressure_ref2[mmc], "Mauvaise Pression");
      for (Int32 i = 0; i < nb_dim2; ++i)
        vc.areEqual(mat_pressure_array[mmc][i], mat_pressure_array_ref2[mmc][i], "Mauvais Tableau de Pression");
    }
    bool is_ok = false;
    try {
    ...
    if (!is_ok)
      ARCANE_FATAL("Devrait lancer une exception pour la conversion 'MaterialVariableCellInt32'");
    is_ok = false;
    try {
    ...
    if (!is_ok)
      ARCANE_FATAL("Devrait lancer une exception pour la conversion 'MaterialVariableCellArrayInt32'");
  }
}
...
_checkIndexedSelection()
{
  ...
  for (Int32 i = 0; i < n; ++i)
    if (selection_view[i] != i)
      ARCANE_FATAL("Mauvais index de sélection={0} valeur={1} (attendu={2})", i, selection_view[i]);
}
