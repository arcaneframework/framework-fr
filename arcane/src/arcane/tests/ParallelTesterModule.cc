...
/*!
 * \brief Classe pour tester les familles de particules.
 */
...
, public IExtraGhostParticlesBuilder
{
 ...
 public:
  ...
  void computeExtraParticlesToSend() override
  {
    ...
    if (pm->isParallel()) {
      // Cette méthode est appelée à partir d'un appel à IPrimaryMesh::updateGhostLayers()
      // et la famille de mailles peut avoir changé, nous devons donc
      // nous assurer que endUpdate() a été appelé.
      // NOTE GG: vérifier si cela doit être fait directement dans DynamicMesh
...
_doInit()
{
...

  {
    ...
    //    pf2->setHasUniqueIdMap(false); // pour voir pourquoi cela. Ne peut pas fonctionner lorsque USE_GRAPH_CONNECTIVITY_POLICY est activé.
...
testLoop()
{
  ...
  if (m_mesh_partitioner) {
    info() << "Définir le partitionneur de maillage";
...
_testSynchronize()
{
  info() << "Test de synchronisation";
  {
    info() << "Début de la création de variable";
    VariableCellArrayReal cells(VariableBuildInfo(this, "Toto"));
    cells.resize(5);
    info() << "Fin de la création de variable";
  }
  ...
  info() << "Initialisation ArrayNode nb_node=" << nbNode();
  m_array_nodes.initialize();
  info() << "Initialisation ArrayFace nb_face=" << nbFace();
  m_array_faces.initialize();
  info() << "Initialisation ArrayCell nb_cell=" << nbCell();
  ...
  // Vérifie les valeurs
  {
    ...
    if (nb_error != 0)
      ARCANE_FATAL("Erreur dans le test de synchronisation: n={0}", nb_error);
  }
...
_testPartialSynchronize()
{
...

  {
    ...
    if (nb_error != 0)
      ARCANE_FATAL("Erreur dans le test de synchronisation partielle: n={0}", nb_error);

    info() << "SYNCHRONISATION PARTIELLE OK.";
  }
}
...
_testPartialMultiSynchronize()
{
...

  {
    ...
    info() << "SYNCHRONISATION MULTIPLE PARTIELLE OK.";
  }
}
...
_testSameValuesOnAllReplica()
{
  info() << "Test si les répliques sont synchronisées";
  ...
  }
  {
    ENUMERATE_CELL (icell, mesh->allCells()) {
      if (icell.index() > 10)
        break;
      Cell cell = *icell;
      info() << "VALEUR maille=" << cell.uniqueId() << " v=" << m_cells.m_real[icell];
    }
  }
  // Vérifie les valeurs
  {
    ...
    if (nb_error != 0)
      fatal() << "Erreur dans le test checkReplicaAreSynced: n=" << nb_error;
  }
}
...
_testDifferentValuesOnAllReplica()
{
  ...
  info() << "Test de valeurs différentes sur réplique";
...

  {
    ...
    nb_error += m_array_cells.checkReplica();
    info() << "NB ERROR3=" << nb_error;
    info() << "Test CheckReplicaAreSynced: n=" << nb_error << " attendu=" << nb_expected_error;
    if (nb_error != nb_expected_error)
      ARCANE_FATAL("Erreur dans le test checkReplicaAreSynced: n={0} attendu={1}",
...
_testMultiSynchronize()
{
  info() << "Test de synchronisation multiple";
