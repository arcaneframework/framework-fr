...
class Span2Impl
{
 ...
 public:
  ...
  {}
  //! Crée une vue 2D vide.
  Span2Impl() = default;
  // Constructeur à partir d'un ConstArrayView. Ceci n'est autorisé
  // que si T est const.
  template <typename X, typename = std::enable_if_t<std::is_same_v<X, value_type>>>
  Span2Impl(const ConstArray2View<X>& from)
  : m_ptr(from.data())
  , m_dim1_size(from.dim1Size())
  , m_dim2_size(from.dim2Size())
  {}
...
: public Span2Impl<T, Int32, Extent1, Extent2>
{
 ...
 public:
  ...
  //! Crée une vue 2D de dimension [\a dim1_size][\a dim2_size]
  ...
  : BaseClass()
  {}
  //! Constructeur de copie à partir d'une autre vue
